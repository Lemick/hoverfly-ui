import type { HoverflySimulation, Request } from '../types/hoverfly';
...
export function getRequestHeader(request: Request) {
  ...
  const destination =
    (request?.destination?.length && request.destination[0].value) || '';
  const path = (request?.path?.length && request.path[0].value) || '';

  return `${method} ${scheme} ${destination} ${path}`.trim();
}

export function initHoverflySimulation(
  initRequestResponsePair: boolean,
): HoverflySimulation {
  return {
    meta: {
      schemaVersion: 'v5.2',
